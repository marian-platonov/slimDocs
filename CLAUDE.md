...
4. **Streamlit UI (`main()`, bottom of file)** — sidebar drives input mode (Single File / Folder / URLs), output format (`_FORMAT_OPTIONS`: md/txt/json/duckdb), and output destination; three or four tabs render results (File Processing, Statistics & Reports, Logs, and DuckDB Explorer — the last only appears when `duckdb` format is selected). All app state lives in `st.session_state`, initialized once in `_init_session()`.
   ...
   - Every run's full result dict is kept in `st.session_state.session_history` (keyed by `run_ts`), not just the latest — Statistics & Reports shows `session_history[selected_session_ts]` when set, else the latest `results`. Clicking a Logs row fires `_on_logs_row_select` (passed as `on_select` on the Logs `st.dataframe`, keyed `"logs_table"`), which sets `selected_session_ts` and a one-shot `show_nav_hint` flag telling the user to open Statistics & Reports themselves. The outer `st.tabs(...)` is deliberately plain (no `key`/`on_change`) — two earlier attempts at auto-switching the active tab from Python (a `components.html` DOM-click script, then `st.tabs(key=..., on_change="rerun")` tracking) both caused real bugs in practice: the DOM script never reliably worked, and `on_change="rerun"` made every tab click trigger a full rerun, which interrupted in-progress extractions and, in one observed case, reset the active tab back to the first one on an unrelated rerun (e.g. using the DuckDB Explorer search box). Don't reintroduce tab-switch tracking without solving both of those first.
